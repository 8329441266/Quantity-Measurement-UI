...
leftSelect = (TakeValue) => {
    $('#leftMeasure').empty();
    $.each(TakeValue, function (key, value) {
        var containt = `<option>${value}</option>`;
        $('#leftMeasure').append(containt );
    });
}

rightSelect = (TakeValue) => {
    $('#rightMeasure').empty();
    $.each(TakeValue, function (key, value) {
        var containt = `<option>${value}</option>`;
        $('#rightMeasure').append( containt);
    });
    ...
    switch (funcOption) {
        case 'Meter':
            ...
            case 'Inch': if (funcOption2 == "Meter") {
                ...
                break;
            case 'Fahrenheit': if (funcOption2 == "Celcius") {
                value = (funcValue - 32) / 1.8;
                console.log(value);
            }
                break;
            case 'Celcius': if (funcOption2 == "Fahrenheit") {
